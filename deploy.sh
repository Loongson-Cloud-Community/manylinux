...
docker tag "${TAG}:${COMMIT_SHA}" "ghcr.io/loongson-cloud-community/${POLICY}_${PLATFORM}:${BUILD_ID}"
docker tag "${TAG}:${COMMIT_SHA}" "ghcr.io/loongson-cloud-community/${POLICY}_${PLATFORM}:latest"

set +x

if [ $DRY_RUN -eq 0 ]; then
  docker push "ghcr.io/loongson-cloud-community/${POLICY}_${PLATFORM}:${BUILD_ID}"
  docker push "ghcr.io/loongson-cloud-community/${POLICY}_${PLATFORM}:latest"
fi

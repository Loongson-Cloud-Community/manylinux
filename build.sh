...
if [ "${POLICY}" == "manylinux2014" ]; then
	...
	BASEIMAGE="lcr.loongnix.cn/openanolis/anolisos:23.4"
	DEVTOOLSET_ROOTPATH="/opt/rh/gcc-toolset-14/root"
	PREPEND_PATH="/usr/local/bin:${DEVTOOLSET_ROOTPATH}/usr/bin:"
	LD_LIBRARY_PATH_ARG="${DEVTOOLSET_ROOTPATH}/usr/lib64:${DEVTOOLSET_ROOTPATH}/usr/lib:${DEVTOOLSET_ROOTPATH}/usr/lib64/dyninst:${DEVTOOLSET_ROOTPATH}/usr/lib/dyninst"
	...
	BASEIMAGE="registry.alpinelinux.org/img/alpine:3.22"
	DEVTOOLSET_ROOTPATH=
	PREPEND_PATH=
	LD_LIBRARY_PATH_ARG=
else
...
if [ "${MANYLINUX_BUILD_FRONTEND}" == "docker" ]; then
	...
	if [ "${PLATFORM}" == "loongarch64" ]; then
		USE_LOCAL_CACHE=0
		CACHE_STORE="--cache-to=type=registry,ref=ghcr.io/loongson-cloud-community/manylinux-cache:${POLICY}_${PLATFORM}_main,mode=max,compression=zstd,compression-level=22"
	fi
	docker buildx build \
		--load \
		"--cache-from=type=registry,ref=ghcr.io/loongson-cloud-community/manylinux-cache:${POLICY}_${PLATFORM}_main" \

...
if [ "${AUDITWHEEL_POLICY}" == "manylinux2014" ]; then
	...
	if [ "${AUDITWHEEL_ARCH}" == "loongarch64" ]; then
		dnf -y install cracklib-dicts dejavu-sans-fonts info langpacks-core-font-en libpciaccess-devel pcre-devel perl-core perl-FindBin perl-IO-Socket-SSL perl-Mozilla-CA perl-NDBM_File perl-subs tar
	fi
elif [ "${OS_ID_LIKE}" == "debian" ]; then
